...
int get_student(int fd, int id, student_t *s)
{
    ...
    ssize_t fileData = read(fd, s, STUDENT_RECORD_SIZE);
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    int studentExists = get_student(fd, id, &exists);
    if (studentExists == NO_ERROR) {
        printf(M_ERR_DB_ADD_DUP, id);
        return ERR_DB_OP;
    } else if (studentExists == ERR_DB_FILE) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
    ...
    newStudent.fname[sizeof(newStudent.fname) - 1] = '\0';
    strncpy(newStudent.lname, lname, sizeof(newStudent.lname) - 1);
    newStudent.lname[sizeof(newStudent.lname) - 1] = '\0';
    newStudent.gpa = gpa;
    ...
    ssize_t dataWritten = write(fd, &newStudent, STUDENT_RECORD_SIZE);
    if (dataWritten != STUDENT_RECORD_SIZE) {
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
...
{
    student_t exists;
    int studentDeleted = get_student(fd, id, &exists);
    if (studentDeleted == SRCH_NOT_FOUND) {
        printf(M_STD_NOT_FND_MSG, id);
        return ERR_DB_OP;
    } else if (studentDeleted == ERR_DB_FILE) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
    ...
    ssize_t dataWritten = write(fd, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE);
...
int print_db(int fd)
{
    ...
    while (1) {
        ssize_t bytesRead = read(fd, &studentData, STUDENT_RECORD_SIZE);
        if (bytesRead == -1) {
            printf(M_ERR_DB_READ);
            return ERR_DB_FILE;
        } else if (bytesRead == 0) {
        ...
        if (memcmp(&studentData, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
            if (!isHeader) {
                printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST_NAME", "LAST_NAME", "GPA");
                isHeader = true;
            }
            float gpa = studentData.gpa / 100.0;
            printf(STUDENT_PRINT_FMT_STRING, studentData.id, studentData.fname, studentData.lname, gpa);
        }
    }
    if (!isHeader) {
        printf(M_DB_EMPTY);
