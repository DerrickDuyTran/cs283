...
int exec_remote_cmd_loop(char *address, int port) {
    ...
    while (1) 
    {
        ...
        size_t sent = send(cli_socket, cmd_buff, send_len, 0);
        ...
        while (1) {
            ssize_t bytes_received = recv(cli_socket, rsp_buff, RDSH_COMM_BUFF_SZ, 0);
            if (bytes_received < 0) {
                perror("recv");
            ...
            is_eof = (rsp_buff[bytes_received - 1] == RDSH_EOF_CHAR);
            if (is_eof) {
                rsp_buff[bytes_received - 1] = '\0';
                bytes_received--;
                if (bytes_received > 0) {
                    printf("%.*s", (int)bytes_received, rsp_buff);
                }
